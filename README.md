...
## Running demoprep in Quarkus dev mode
...
```
(demoprep should be included in the returned array)
...
[demoprep-schema.json](./src/main/resources/schemas/demoprep-schema.json ), run the following to create an
instance of the demoprep workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/demoprep -d@input.json
```
